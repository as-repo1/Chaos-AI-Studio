...
The core of the studio is `app.py`. It does **not** rely on `systemd` or Docker. 

Instead, it uses a background python `threading.Thread` known as the **Watchdog**.
...
## 2. Resource Contention (The Sentinel File)
...
To solve this, we enforce **mutual exclusion** for heavy VRAM workloads via Sentinel Files.
- When `Video AI (Wan2.2)` starts, it creates a sentinel file `/tmp/wan_video_active`.
- `Image AI (ComfyUI)` checks for this file. Since both use the underlying ComfyUI engine but require completely different VRAM allocations and model weights, the dashboard knows how to forcefully kill one to allow the other to boot safely.

## 3. Dynamic Model Injection

To avoid hardcoded configs, `start_ai_stack.sh` relies purely on environment variables passed down by `app.py`.
When a user clicks "Start" on Text AI:
1. `app.py` reads `config.json` to find the selected model, threads, and Compute Backend (Vulkan device index).
